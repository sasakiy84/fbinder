...
Markdown の本文では CommonMark に加えて、pipe table と `~~取り消し線~~` を有効にする。raw HTML は引き続き無効化するため、`<br>` などの HTML は文字列として表示される。
Markdown の table は列数が多い場合でも列幅が極端に潰れないよう、横スクロールできる領域で囲む。長いセルは上限幅で折り返す。
...
- 列数が多い場合でも読めるよう、横スクロールできる領域で囲む。長いセルは上限幅で折り返す。
- セルの値は HTML エスケープする。
...
- 詳細ページ: 生成日、Markdown コピーボタン、元ファイルパスのコピーボタンをタイトル直下に控えめに表示し、目次は本文を読む前の補助ナビゲーションとして置く。
